...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            def check_finish(current_bool, finish_position, x, y):
                return current_bool or (x, y) == finish_position

            wah = tuple(check_finish(
                tt[i], self.corners[i], nextx, nexty) for i in range(4))

            succ = ((nextx, nexty), wah)
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    def modified_state(state: Any, num: Number) -> Any:
        pos, tt = state
        l = list(tt)
        l[num] = True
        return (problem.corners[num], tuple(l))

    cds = [util.manhattanDistance(pos, problem.corners[i]) + cornersHeuristic(modified_state(state, i), problem)
           for i in range(4) if not tt[i]]
